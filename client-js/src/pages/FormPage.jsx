...
function FormPage() {
    ...
    useEffect(() => {
        if (!user.user.firstName && Console.end_register) {
            const endRegisterTime = new Date(Console.end_register).getTime();
            const currentTime = new Date().getTime();

            if (endRegisterTime - currentTime < 0) {
                Swal.fire({
                    title: "หมดเวลาการลงทะเบียน",
    ...
    }, [user.user.firstName, Console.end_register]);
                ...
                >
                    {({ errors, touched }) => (
                        <Form className="p-4 w-full mx-auto flex flex-col rounded-xl space-y-3 sm:shadow-2xl mt-24 pb-4 mb-10 bg-white">
                            <p className="text-center font-bold text-primary-500 text-xl mb-4">{sectionName[currentSection]}</p>
